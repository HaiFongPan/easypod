...
class TranscribePayload(BaseModel):
  audio_path: Path = Field(..., description='Absolute path to audio file')
  options: Dict[str, Any] = Field(default_factory=dict)
...
def _load_model(payload: InitializePayload) -> Any:
  ...
  if vad_model:
    ...
    if 'max_single_segment_time' in options:
      vad_kwargs.setdefault('max_single_segment_time', options['max_single_segment_time'])
    if vad_kwargs:
...
def _prepare_generate_kwargs(audio: Path, options: Dict[str, Any]) -> Dict[str, Any]:
  opts = options or {}
  kwargs: Dict[str, Any] = {
    'input': str(audio),
  }
...
def _segment_text(res_item: Dict[str, Any]) -> List[Dict[str, Any]]:
  ...
  if isinstance(sentence_info, list) and sentence_info:
    for sent in sentence_info:
      ...
      if text and start is not None and end is not None:
        segments.append({'text': text, 'start_sec': float(start), 'end_sec': float(end)})
    if segments:
      return segments
  ...
  if isinstance(stamp_sents, list):
    for sent in stamp_sents:
      ...
      if start is None or end is None:
        continue
      segments.append({'text': f'{text_seg}{punc}'.strip(), 'start_sec': float(start) / 1000.0, 'end_sec': float(end) / 1000.0})
    if segments:
      return segments
  ...
  if isinstance(timestamp, list) and timestamp and text:
    ...
    segments.append({'text': text, 'start_sec': float(start_ms) / 1000.0, 'end_sec': float(end_ms) / 1000.0})
  elif text:
...
def _run_transcription(task_id: str, payload: TranscribePayload) -> None:
  LOGGER.info('Starting transcription task %s for audio=%s', task_id, payload.audio_path)
  ...
  try:
    ...
    if not results:
      raise RuntimeError('FunASR did not return any results for the provided audio')
    ...
    LOGGER.info('Transcription task %s completed with %d segments', task_id, len(formatted_segments))
  except Exception as exc:  # pragma: no cover - resilience path
    LOGGER.exception('Transcription task %s failed: %s', task_id, exc)
    STATE.tasks[task_id].update({'status': 'failed', 'error': str(exc), 'progress': 1.0})
...
def transcribe(payload: TranscribePayload) -> Dict[str, Any]:
  ...
  thread = threading.Thread(target=_run_transcription, args=(task_id, payload), daemon=True)
  thread.start()
